...
export function CartPredictTask({
  question,
  outcome,
  reason,
  committedLabel,
  ...
  onReasonChange,
  onCommit,
}: CartPredictTaskProps) {
  return (
    <div className="space-y-4" data-testid="cart-predict-task">
      <p className="text-sm leading-relaxed text-[var(--ink-muted)]">
        {CART_COPY.predictInstruction}
      </p>
      <Card className="space-y-5 p-4">
        ...
        <fieldset className="space-y-3" disabled={locked}>
          ...
          {CART_PREDICT_OUTCOMES.map((option) => (
            <label
              key={option.value}
        ...
        <div className="space-y-2">
          <label htmlFor="cart-predict-reason" className="text-sm font-medium text-[var(--ink)]">
            {CART_COPY.reasonLabel}
          </label>
          <textarea
            id="cart-predict-reason"
            value={reason}
            onChange={(event) => onReasonChange(event.target.value)}
            placeholder={CART_COPY.reasonPlaceholder}
            disabled={locked}
        ...
        {needMore ? (
          <p className="text-sm text-[var(--ink-muted)]">{CART_COPY.predictNeedBoth}</p>
        ) : null}
        {committedLabel ? (
          <p className="text-sm text-[var(--ink-muted)]">
            {CART_COPY.predictLocked}：{committedLabel}
          </p>
        ) : null}
        {!locked ? (
          <div className="flex justify-end">
            <Button onClick={onCommit}>{CART_COPY.predictSubmit}</Button>
          </div>
        ) : null}
      </Card>
    </div>
